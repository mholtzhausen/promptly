...
fn main() {
    ...
    println!("cargo:rerun-if-changed={}", frontend.join("package-lock.json").display());
    println!("cargo:rerun-if-changed={}", frontend.join("index.html").display());
    println!("cargo:rerun-if-changed={}", frontend.join("vite.config.ts").display());
